...
describe("streams", () => {
  ...
  describe("peel", () => {
    ...
    it("should handle empty arrays", () => {
      ...
      return expectExit(s.collectArray(s.chain(s2, ([_h, r]) => r)), done([]));
    });
  ...
  describe("switchLatest", () => {
    it("should produce the latest elements", () => {
      ...
      const s2 = s.as(s1, s.concat(s.take(s.periodically(10), 2), s.never as Stream<never, number>));
      // A third stream that emits 3 elements
      const after30 = wave.as(wave.after(50), s.take(s.periodically(20), 4));
