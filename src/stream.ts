...
function interruptFiberSlot(slot: Ref<Option<Fiber<never, void>>>): Wave<never, void> {
  return wave.chain(slot.get, (optFiber) =>
    pipe(optFiber, o.fold(() => wave.pure(undefined as void), (f) => f.interrupt)))
}

function waitFiberSlot(slot: Ref<Option<Fiber<never, void>>>): Wave<never, void> {
  return wave.chain(slot.get, (optFiber) =>
    pipe(optFiber, o.fold(() => wave.pure(undefined as void), (f) => wave.asUnit(f.wait)))
  )
}
